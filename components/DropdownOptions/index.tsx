  ...
  return (
    <>
      <section className="flex justify-evenly items-center py-5">
        {/* Shadcdn */}
        <div className="px-7 py-4 border-zinc-100 border-2 rounded-xl w-1/3 mx-4">
          <DropdownMenu>
            ...
            <DropdownMenuContent className="overflow-y-scroll">
              <DropdownMenuLabel>{config.category.name ? config.category.name : "Category"}</DropdownMenuLabel>
              <DropdownMenuSeparator />
              {
        ...
        <div className="px-7 py-4 border-zinc-100 border-2 rounded-xl w-1/3 mx-4">
          <DropdownMenu>
            <DropdownMenuTrigger className="flex outline-none justify-between w-full">
              {config.level ? config.level : "Difficulty"} <ChevronDown />{" "}
            </DropdownMenuTrigger>
            <DropdownMenuContent>
              <DropdownMenuLabel> {config.level ? config.level : "Difficulty"}</DropdownMenuLabel>
              <DropdownMenuSeparator />
             {
        ...
        <div className="px-7 py-4 border-zinc-100 border-2 rounded-xl w-1/3 mx-4">
          <DropdownMenu>
            ...
            <DropdownMenuContent>
              <DropdownMenuLabel>{config.type ? config.type : "Type"}</DropdownMenuLabel>
              <DropdownMenuSeparator />
             {
